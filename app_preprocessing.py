...
# 💡 新機能：官公庁Excel対策（上部の不要行スキップと、複数行ヘッダーの結合）
...
def load_data(file, skip, head):
    if file.name.endswith('.csv'): 
        df = pd.read_csv(file, skiprows=skip, header=list(range(head)), dtype=str)
    else: 
        df = pd.read_excel(file, skiprows=skip, header=list(range(head)), dtype=str)
    
    # 複数行ヘッダーの場合、「男_40-44歳」のようにアンダーバーで結合して1行にする
...
if uploaded_file is not None:
    if st.session_state.main_filename != uploaded_file.name:
        try:
            ...
            st.session_state.main_filename = uploaded_file.name
...
if st.session_state.current_df is not None:
        ...
        with col2:
            ...
            if st.button("欠損値を処理して上書きする", type="primary"):
                ...
                if "削除" in na_method:
                    ...
                    elif "直前" in na_method: st.session_state.current_df[na_col] = df[na_col].ffill() # 💡 これが ffll (前方補完) です
    ...
    with tab4:
        ...
        if transform_mode.startswith("A"):
            ...
            st.info("💡 横にズラッと並んだ「男_40歳」「女_40歳」などの列を、縦に折りたたみます。")
